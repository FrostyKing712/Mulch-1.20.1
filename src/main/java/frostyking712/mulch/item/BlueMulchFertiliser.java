...
public class BlueMulchFertiliser extends Item {
    public BlueMulchFertiliser(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide) {  // Server-side only
            ...
            for (int dx = -13; dx <= 13; dx++) {  // Loop over X (-13 to +13)
                for (int dz = -13; dz <= 13; dz++) {  // Loop over Z (-13 to +13)
