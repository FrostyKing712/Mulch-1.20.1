...
public class BlackMulchFertiliser extends Item {
    public BlackMulchFertiliser(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide) {  // Server-side only
            ...
            for (int dx = -11; dx <= 11; dx++) {  // Loop over X (-11 to +11)
                for (int dz = -11; dz <= 11; dz++) {  // Loop over Z (-11 to +11)
