...
public class AmberMulchFertiliser extends Item {
    public AmberMulchFertiliser(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide) {  // Server-side only
            ...
            for (int dx = -5; dx <= 5; dx++) {  // Loop over X (-5 to +5)
                for (int dz = -5; dz <= 5; dz++) {  // Loop over Z (-5 to +5)
