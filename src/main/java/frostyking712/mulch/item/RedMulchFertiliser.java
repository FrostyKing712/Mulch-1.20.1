...
public class RedMulchFertiliser extends Item {
    public RedMulchFertiliser(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide) {  // Server-side only
            ...
            for (int dx = -9; dx <= 9; dx++) {  // Loop over X (-9 to +9)
                for (int dz = -9; dz <= 9; dz++) {  // Loop over Z (-9 to +9)
