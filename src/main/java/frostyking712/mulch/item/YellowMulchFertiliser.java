...
public class YellowMulchFertiliser extends Item {
    public YellowMulchFertiliser(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide) {  // Server-side only
            ...
            for (int dx = -3; dx <= 3; dx++) {  // Loop over X (-3 to +3)
                for (int dz = -3; dz <= 3; dz++) {  // Loop over Z (-3 to +3)
