...
public class BrownMulchFertiliser extends Item {
    public BrownMulchFertiliser(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide) {  // Server-side only
            ...
            for (int dx = -1; dx <= 1; dx++) {  // Loop over X (-1 to +1)
                for (int dz = -1; dz <= 1; dz++) {  // Loop over Z (-1 to +1)
